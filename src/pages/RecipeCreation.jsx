...
const LICENSES = [
    { id: 1, code: "cc0", label: "CC0", desc: "Public Domain - no restrictions" },
    { id: 2, code: "cc-by", label: "CC BY", desc: "Credit required" },
    { id: 3, code: "cc-by-nc", label: "CC BY-NC", desc: "Non-commercial use only" },
    { id: 4, code: "cc-by-nd", label: "CC BY-ND", desc: "No derivatives allowed" },
    { id: 5, code: "cc-by-nc-nd", label: "CC BY-NC-ND", desc: "Most restrictive CC license" },
    { id: 6, code: "all_rights_reserved", label: "All Rights", desc: "Full copyright reserved" },
];
...
const BLANK = {
    title: "", body: "", prepTime: "", servings: "", tags: [], customTags: [], licenseId: null,
...
/* ═══════════════════════════════════════════════════════════
   STAGE 0 - DETAILS
═══════════════════════════════════════════════════════════ */
function StageDetails({ data, setData, licenses }) {
...
/* ═══════════════════════════════════════════════════════════
   STAGE 1 - INGREDIENTS
═══════════════════════════════════════════════════════════ */
function StageIngredients({ data, setData }) {
    const add = () => setData(d => ({ ...d, ingredients: [...d.ingredients, { name: "", quantity: "", unit: "", is_animal: false, is_allergen: false }] }));
    const upd = (i, f, v) => setData(d => { const n = [...d.ingredients]; n[i] = { ...n[i], [f]: v }; return { ...d, ingredients: n }; });
    const del = i => setData(d => ({ ...d, ingredients: d.ingredients.filter((_, j) => j !== i) }));

    return (
        <div className="flex flex-col gap-3">
            <p className="text-neutral-300 text-xs leading-relaxed">
                Add each ingredient. Flag allergens and animal-derived ones - readers with dietary needs will see them.
            </p>
            {data.ingredients.map((ing, i) => (
                <IngCard key={i} ing={ing} idx={i} isNew={i === data.ingredients.length - 1 && !ing.name} onUpd={upd} onDel={del} />
            ))}
...
function IngCard({ ing, idx, isNew, onUpd, onDel }) {
    ...
    const hasFlags = ing.is_allergen || ing.is_animal;

    return (
        <div className={`fk-card overflow-hidden ${open ? "border-neutral-300!" : ""}`} style={open ? { borderColor: "#252525" } : {}}>
            {/* Header row */}
...
/* ═══════════════════════════════════════════════════════════
   STAGE 2 - STEPS
═══════════════════════════════════════════════════════════ */
function StageSteps({ data, setData, plan }) {
    const lim = PLAN_LIMITS[plan] || PLAN_LIMITS.FREE;

    const add = () => setData(d => ({ ...d, steps: [...d.steps, { instruction: "", technique: "", estimated_minutes: "", imageFile: null }] }));
    const upd = (i, f, v) => setData(d => { const n = [...d.steps]; n[i] = { ...n[i], [f]: v }; return { ...d, steps: n }; });
    const del = i => setData(d => ({ ...d, steps: d.steps.filter((_, j) => j !== i) }));
    const mv = (i, dir) => setData(d => {
    ...
    return (
        <div className="flex flex-col gap-4">
            <p className="text-neutral-300 text-xs leading-relaxed">
                Each step is a moment. Tag the technique, note time{lim.stepImages ? " - PRO: add a step photo" : ""}.
            </p>
            {data.steps.map((step, i) => (
                <StepCard key={i} step={step} idx={i} total={data.steps.length}
                    isNew={i === data.steps.length - 1 && !step.instruction}
                    onUpd={upd} onDel={del} onMv={mv} canStepImage={lim.stepImages} />
            ))}
            {/* Add button offset to align with cards (past the number column) */}
            <div className="flex gap-3">
                <div className="w-8 flex-shrink-0" />
                <AddBtn onClick={add} label="Add step" className="flex-1" />
            </div>
        </div>
    );
}
    ...
    return (
        <div className="flex gap-3 items-start">
          ...
          ${open ? "bg-amber-500 text-black step-num-open" : "bg-[#111] border border-[#222] text-amber-500/40"}`}>
            ...
            {/* Card */}
            <div className="flex-1 min-w-0">
                <div className={`fk-card overflow-hidden ${open ? "border-[#252525]!" : ""}`} style={open ? { borderColor: "#252525" } : {}}>
                    {/* Preview header */}
...
function StageMedia({ data, setData, plan }) {
    ...
    const addImages = files => {
        ...
        if (data.imageFiles.length + arr.length > lim.images) { alert(`${plan} plan: max ${lim.images} images.`); return; }
        setData(d => ({ ...d, imageFiles: [...d.imageFiles, ...arr.map(f => ({ file: f, url: URL.createObjectURL(f), name: f.name }))] }));
    };

    const addVideos = files => {
        if (!lim.allowVideos) { alert("Video uploads require PRO or ORG plan."); return; }
        const arr = Array.from(files).filter(f => f.type.startsWith("video/"));
        if (data.videoFiles.length + arr.length > lim.videos) { alert(`${plan} plan: max ${lim.videos} videos.`); return; }
    ...
    const onThumbDragStart = (e, i) => {
        ...
        // Mark as internal drag so the drop zone ignores it
        e.dataTransfer.setData("thumb", "1");
    };
    ...
    // Zone drag — only fires for external files, not internal thumb drags
    const onZoneDragOver = (e) => {
        if (e.dataTransfer.types.includes("thumb")) return;
        e.preventDefault();
        setImgOver(true);
    };
    const onZoneDrop = (e) => {
        ...
        <div className="flex flex-col gap-6">
            ...
            <div className="flex items-center gap-2.5 px-4 py-3 rounded-2xl border border-[#161616] bg-[#0a0a0a]">
                ...
                <div className="ml-auto flex items-center gap-3 text-[10px] text-neutral-500">
                    <span>{lim.images} img{lim.images !== 1 ? "s" : ""}</span>
                    {lim.allowVideos && <span>· {lim.videos} vid{lim.videos !== 1 ? "s" : ""}</span>}
                    {!lim.allowVideos && <span>· No videos</span>}
                        ...
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                            {data.imageFiles.map((img, i) => (
                                <div
                                    ...
                                    style={{
                                        borderColor: dropIdx === i && dragIdx !== i
                                            ? "rgba(232,160,32,0.6)"
                                            : dragIdx === i
                                ...
                                >
                                    ...
                                    {/* Cover badge — inside media-thumb which is position:relative via CSS */}
                                    {i === 0 && (
            ...
            {/* Videos */}
            <div>
                <SectionDivider icon="▷" label="Videos"
                    sub={lim.allowVideos ? `${data.videoFiles.length} of ${lim.videos}` : "PRO · ORG only"} />
                <div className="mt-3">
                    <div
                        onDragOver={e => { e.preventDefault(); if (lim.allowVideos) setVidOver(true); }}
                        onDragLeave={() => setVidOver(false)}
                        onDrop={e => { e.preventDefault(); setVidOver(false); if (lim.allowVideos) addVideos(e.dataTransfer.files); }}
                        onClick={() => lim.allowVideos && vidRef.current?.click()}
                        className={`border border-dashed rounded-2xl px-6 py-8 flex flex-col items-center gap-3 transition-all duration-300
                            ${!lim.allowVideos
                                ? "border-[#111] opacity-35 cursor-not-allowed"
                                : vidOver
                                    ? "dropzone-over cursor-copy"
                                    : "border-[#181818] cursor-pointer hover:border-amber-500/60"}`}
                    >
                        ...
                        <div className="text-center">
                            <p className={`text-xs transition-colors ${vidOver ? "text-amber-400/60" : "text-neutral-300"}`}>
                                {lim.allowVideos
                                    ...
                                    : "Upgrade to PRO or ORG to upload videos"}
                            </p>
                            {lim.allowVideos && <p className="text-[10px] text-neutral-400 mt-1">MP4 · MOV · WEBM · max 200 MB each</p>}
                        </div>
                    </div>
...
/* ═══════════════════════════════════════════════════════════
   STAGE 4 - PUBLISH
═══════════════════════════════════════════════════════════ */
function StagePublish({ data, onPublish, onDraft, submitting, error }) {
    ...
    return (
        <div className="fk-root min-h-screen flex items-center justify-center px-6">
            ...
            <div className="text-center max-w-xs mx-auto relative z-10">
                <div className="w-24 h-24 rounded-full bg-amber-500/10 border border-amber-500/20
...
export default function RecipeCreate() {
    ...
    const submit = async (publish) => {
        ...
        // Step images could be sent as separate fields or embedded - left for backend integration
        ...
        <div className="fk-root min-h-screen flex flex-col">
            ...
            <div className="relative z-10 px-4 sm:px-6 pt-4 pb-0 flex-shrink-0">
                <p key={stage} className="text-neutral-400 text-[10px] italic fade-up">
                    {["Give your recipe a name worth forking.",
                        "What goes in - and what readers need to know.",
                        "Each step is a moment. Own the technique.",
                        "Show the world what it looks like.",
                        "One last look before it hits the bench."][stage]}
                </p>
            </div>

            {/* ── Main content ── */}
            <div className="relative z-10 flex-1">
                <div key={animKey} className="stage-enter max-w-xl mx-auto w-full px-4 sm:px-6 pt-6 pb-8">
                    {stage === 0 && <StageDetails data={data} setData={setData} licenses={licenses} />}
                    {stage === 1 && <StageIngredients data={data} setData={setData} />}
                    {stage === 2 && <StageSteps data={data} setData={setData} plan={USER_PLAN} />}
                    {stage === 3 && <StageMedia data={data} setData={setData} plan={USER_PLAN} />}
                    {stage === 4 && <StagePublish data={data} onPublish={() => submit(true)} onDraft={() => submit(false)} submitting={submitting} error={submitError} />}

                    {/* ── Inline nav (not sticky) ── */}
                    {stage < 4 && (
                  ...
                  active:scale-[0.99] transition-all duration-300">
                                {stage === 3 ? "Review & publish →" : "Continue →"}
                            </button>
                        </div>
                    )}
                </div>
