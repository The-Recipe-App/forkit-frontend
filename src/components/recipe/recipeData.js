...
/**
 * useRecipeFeed
 * @param {{ sort?, page?, pageSize?, q?, authorId?, isDraft?, licenseCode? }} params
 * @returns {{ data: { items: NormalizedRecipe[], pagination: object } | null, loading, error, reload }}
 */
export function useRecipeFeed(params = {}) {
    ...
    return useAsyncData(
        async (signal) => {
            ...
            const json = await apiFetch(`/recipes/feed/list?${qs}`, signal);
            return {
...
/**
 * useRecipe — single recipe for detail page.
 *
 * GET /{id} returns  { ok: true, recipe: { ... } }
 * We unwrap the envelope before normalizing so the rest of the app
...
export function useRecipe(id) {
    return useAsyncData(
        async (signal) => {
            const json = await apiFetch(`/recipes/${id}`, signal);
            // Unwrap envelope: { ok: true, recipe: {...} }
...
/**
 * useRecommendations
 * @param {number|null} recipeId  seed recipe
 * @param {number}      limit
 */
export function useRecommendations(recipeId = null, limit = 8) {
    return useAsyncData(
        async (signal) => {
            ...
            const json = await apiFetch(`/recipes/feed/recommendations?${qs}`, signal);
...
/**
 * useTrendingPreview — lightweight widget data
 * @param {number} limit
 */
export function useTrendingPreview(limit = 5) {
    return useAsyncData(
        async (signal) => {
            const json = await apiFetch(`/recipes/feed/trending-preview?limit=${limit}`, signal);
            return json.items ?? [];
        },
        [limit],
    );
